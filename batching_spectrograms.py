...
def wh_from_sample(dirname, sess=None):
    ...
    spectrogram, height, width = read_and_decode(filename_queue)
    if sess==None:
...
def spec_batch(dirname, sess=None, batch_size=1, num_threads=4, min_after_dequeue=100):
    specs = glob('{}/*.spec'.format(dirname))
...
class SpecData:
    ...
    def __init__(self, session, dirname, batch_size):
        self.sess = session
        self.spec_batch = spec_batch(dirname, sess=session, batch_size=batch_size)
