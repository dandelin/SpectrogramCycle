...
try:
    ...
    batch_epoch = max(len(a_data_pool), len(b_data_pool)) // batch_size
    ...
    for it in range(sess.run(it_cnt), max_it):
        ...
        # save
        if (it) % 500 == 0:
            save_path = saver.save(sess, '%s/it(%d)_Epoch_(%d)_(%dof%d).ckpt' % (ckpt_dir, it, epoch, it_epoch, batch_epoch))
        ...
        if (it) % 500 == 0:
            ...
            for i, (name, sample) in enumerate(zip(['a', 'ab', 'aba', 'b', 'ba', 'bab'], samples)):
                batching_spectrograms_single.save_reconstructed_audio(sample, '{}/it{}_Epoch{}_{}of{}_{}.wav'.format(save_dir, it, epoch, it_epoch, batch_epoch, name))
